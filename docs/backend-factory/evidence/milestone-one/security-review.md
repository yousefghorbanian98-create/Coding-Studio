...
| 15 | Uncontrolled network access | Boundary code performs **no networking**; the CI probe fetches the pinned artifact from its version-scoped URL and verifies SHA-256 (the immutable trust anchor) before a non-network command; telemetry disabled via explicit env; forbidden packages audited (`validate:mission` gate) |
